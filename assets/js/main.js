...
(function () {
  ...
  function initContentPolish(){
    var path=window.location.pathname.split("/").pop()||"index.html";
    if(path==="wiedza.html"){
      var eyebrow=Array.prototype.find.call(document.querySelectorAll('.eyebrow'),function(el){return el.textContent.trim()==='Formaty';});if(eyebrow)eyebrow.textContent='Zasoby';
    }
    if(path==="media.html"){
      Array.prototype.forEach.call(document.querySelectorAll('section'),function(section){var eyebrow=section.querySelector('.eyebrow');if(eyebrow&&eyebrow.textContent.trim()==='Dla mediów')section.remove();});
    }
    if(path==="o-fundacji.html"){
      Array.prototype.forEach.call(document.querySelectorAll('#ludzie .person'),function(person){var name=person.querySelector('.h4');if(!name||name.textContent.trim()!=='Joanna Borkowska')return;var p=person.querySelector('p:not(.person-role)');if(!p)return;var phrase='Fundację Ruinersi na Dolnym Śląsku',text=p.textContent,idx=text.indexOf(phrase);if(idx===-1)return;var frag=document.createDocumentFragment();frag.appendChild(document.createTextNode(text.slice(0,idx)));var a=document.createElement('a');a.href='https://www.facebook.com/fundacjaruinersi';a.target='_blank';a.rel='noopener noreferrer';a.textContent=phrase;frag.appendChild(a);frag.appendChild(document.createTextNode(text.slice(idx+phrase.length)));p.textContent='';p.appendChild(frag);});
    }
    if(path==="index.html"){
      Array.prototype.forEach.call(document.querySelectorAll('.area .h3'),function(h){if(h.textContent.trim()==='Dotacje i dobre praktyki')h.innerHTML='Dotacje i&nbsp;dobre praktyki';});
    }
    if(path==="spolecznosc.html"){
      Array.prototype.forEach.call(document.querySelectorAll('.areas .area'),function(card){var h=card.querySelector('.h3'),p=card.querySelector('p');if(!h||!p)return;var title=h.textContent.trim();if(title==='Grupy na Facebooku'){p.innerHTML='Przestrzeń wymiany wiedzy, doświadczeń, ogłoszeń i kontaktów między tysiącami osób: <a href="https://www.facebook.com/groups/488002923561276" target="_blank" rel="noopener noreferrer">Ruinersi na Dolnym Śląsku</a> oraz <a href="https://www.facebook.com/groups/1056930299816290" target="_blank" rel="noopener noreferrer">Ruinersi – Stare Domy do Uratowania</a>.';}else if(title==='Fundacja'){p.innerHTML='Formalna organizacja rozwijająca edukację, wydarzenia, projekty i reprezentację społeczną. <a href="https://www.facebook.com/fundacjaruinersi" target="_blank" rel="noopener noreferrer">Profil Fundacji na Facebooku</a>.';}else if(title==='Klaster'){p.innerHTML='Sieć formalnie współpracujących organizacji społecznych i Partnerów Wspierających. Fundacja jest jego inicjatorem i operatorem. <a href="klaster.html">Dowiedz się więcej o Klastrze</a>.';}});
    }
  }
  ...
  ready(function(){initCurrentAssets();initProjectAssets();initFoundationTimelineLinks();initPagePolish();initCommunityLinks();initNav();initReveal();initDates();initFilters();initForms();initSocialFooter();initContentPolish();initYear();});
})();